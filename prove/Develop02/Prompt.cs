using System.Net.NetworkInformation;
using System;

public class Prompt {
    public List<String> _List = new List<String>{
        "Who was the most interesting person I interacted with today?",
        "What was the best part of my day?",
        "How did I see the hand of the Lord in my life today?",
        "What was the strongest emotion I felt today?",
        "If I had one thing I could do over today, what would it be?",
        "What was the most beautiful thing I saw today?",
        "What new thing did I learn today?",
        "How did I set myself up for success?",
        "What spontaneous thing did I do today",
        "Which of my goals did I make progress on today?",
        "Who did I build up today and how?",
        "How did I serve God or serve others today?",
        "What did I pray for today?",
        "How did I help gather scattered Israel today?",
        "What delicious things did I eat today?",
        "What am I looking forward to in the future"
    };


    public String GetRandomPrompt(){
        Random rand = new Random();
        int r = rand.Next(_List.Count);
        return _List[r];
    }




}